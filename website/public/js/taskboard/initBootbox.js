...
function initWysiwyg(context) {
    ...
    if (textarea.length === 1) {
        ...
        var editor = jQuery('' + '<div>' + '<div class="btn-toolbar" data-role="editor-toolbar" data-target="#' + textareaId + '">' + '<div class="btn-group">' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="bold" title="Bold (Ctrl/Cmd+B)"><i class="fa fa-bold"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="italic" title="Italic (Ctrl/Cmd+I)"><i class="fa fa-italic"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="strikethrough" title="Strikethrough"><i class="fa fa-strikethrough"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="underline" title="Underline (Ctrl/Cmd+U)"><i class="fa fa-underline"></i></button>' + '</div>' + '<div class="btn-group">' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="insertunorderedlist" title="Bullet list"><i class="fa fa-list-ul"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="insertorderedlist" title="Number list"><i class="fa fa-list-ol"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="outdent" title="Reduce indent (Shift+Tab)"><i class="fa fa-indent fa-flip-horizontal"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="indent" title="Indent (Tab)"><i class="fa fa-indent"></i></button>' + '</div>' + '<div class="btn-group">' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="justifyleft" title="Align Left (Ctrl/Cmd+L)"><i class="fa fa-align-left"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="justifycenter" title="Center (Ctrl/Cmd+E)"><i class="fa fa-align-center"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="justifyright" title="Align Right (Ctrl/Cmd+R)"><i class="fa fa-align-right"></i></button>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-qtip-options=\'' + qtipopts + '\' data-edit="justifyfull" title="Justify (Ctrl/Cmd+J)"><i class="fa fa-align-justify"></i></button>' + '</div>' + '<div class="btn-group">' + '<div class="btn-group">' + '<button type="button" class="btn btn-default btn-editor tooltipTitle dropdown-toggle" data-toggle="dropdown" data-qtip-options=\'' + qtipopts + '\' title="Hyperlink"><i class="fa fa-link"></i></button>' + '<div class="dropdown-menu col-md-6">' + '<div class="input-group">' + '<input class="form-control" placeholder="URL" type="text" data-edit="createLink"/>' + '<span class="input-group-btn">' + '<button class="btn btn-default" type="button">Add</button>' + '</span>' + '</div>' + '</div>' + '</div>' + '<button type="button" class="btn btn-default btn-editor tooltipTitle" data-edit="unlink" data-qtip-options=\'' + qtipopts + '\' title="Remove Hyperlink"><i class="fa fa-cut"></i></button>' + '</div>' + '</div>' + '<div id="' + textareaId + '" class="editor"></div>' + '</div>');
...
function initMultiSelect(context) {
    $("select[data-multiSelect]", context).each(function() {
        $(this).multiselect();
        alert('init m s');
    });
}
...
function initActionMenu(context, parameters) {
    ...
    var defaultParameters = {
        ...
        animation: false,
        html: true,
        template: '<div class="popover actionMenu" onmouseover="' + mouseOver + '">' +
            '<div class="arrow"></div>' +
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    ...
                    if (result) {
                        ...
                        socket.delete("/Comment/" + commentId, {
                            _csrf: getCsrfToken()
                        }, function( /** sails.json.comment */ comment) {
    ...
    container.on("click", "a[data-remove-link='true']", function(event) {
        ...
        bootbox.confirm({
            ...
            callback: function(result) {
                ...
                if (result) {
                    // Remove comment via socket
                    socket.delete("/Link/" + linkId, {
                        _csrf: getCsrfToken()
                    }, function( /** sails.json.link */ link) {
