...
public class HttpConnectionUtils implements Runnable {
	...
	public void run() {
		...
		try {
			...
			case GET:
				...
				if(null != SESSIONID){ 
					httpGet.setHeader("Cookie", "sails.sid=" + SESSIONID); 
					} 
			...
			case POST:
				...
				if(null != SESSIONID){ 
					httpPost.setHeader("Cookie", "sails.sid=" + SESSIONID); 
					} 
			...
			case PUT:
				...
				if(null != SESSIONID){ 
					httpPut.setHeader("Cookie", "sails.sid=" + SESSIONID); 
					} 
	...
	private void processEntity(HttpEntity entity) throws IllegalStateException,
		...
		for (int i = 0; i < cookies.size(); i++) { 
			if ("sails.sid".equals(cookies.get(i).getName())) { 
	...
	private void processBitmapEntity(HttpEntity entity) throws IOException {
		...
		BitmapWithUrl bwu = new BitmapWithUrl();
		bwu.setBitmap(bm);
		bwu.setUrl(url);
		handler.sendMessage(Message.obtain(handler, DID_SUCCEED, bwu));
	}
